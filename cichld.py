...
if len(sys.argv) != 2:
    print("python3 cichlid.py contract.sol [output]")
    sys.exit(-1)
...
def getModifiersOfFunction(function):
    modifiers = [{'name':i.name,'modifiers':i} for i in function.all_internal_calls() if type(i) == Modifier]
    return modifiers

def getSummaryOfContract(contract):
    contractSummary = []
    for function in contract.functions:
        if(function.contract_declarer == contract and (function.visibility == 'external' or function.visibility == 'public')):
            functionEvents = getEventsOfFunction(function)
            functionModifiers = getModifiersOfFunction(function)
            contractSummary.append({'name':function.name, 'modifiers': functionModifiers, 'events': functionEvents, 'function':function})
                    ...
                    with tag('ul'):
                        for f in functions:
                            with tag('details', klass='function', name=f['name']):
                                ...
                                with tag('ul'):
                                    if(f['function'].view):
                                        with tag('li', klass='mutability', name='mutability'):
                                            text('view function')
                                    if(len(f['modifiers'])>0):
                                        with tag('details', klass='modifiers', name='modifiers'):
                                            with tag('summary'):
                                                text('modifiers')
                                            with tag('ul', klass='modifierlist', name='modifierlist'):
                                                for m in f['modifiers']:
                                                    with tag('li', klass='modifier', name=m['name']):
                                                        text(m['name'])
                                    if(len(f['events'])>0):
                                        with tag('details', klass='events', name='events'):
                                            with tag('summary'):
                                                text('events')
                                            with tag('ul', klass='eventlist', name='eventlist'):
                                                for e in f['events']:
                                                    with tag('li', klass='event', name=e['name']):
                                                        text(e['name'])
